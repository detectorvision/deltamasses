/* $Id: SpectrumCanvas.java 245 2009-01-02 15:53:33Z frank $ */
...
public final class SpectrumCanvas extends Diagram {
	Record record;
	SpectraPair pair;
	boolean isDiscoveryEdition=false;
	int borderLeft = 0; 
	int borderRight = 0;
	int borderTop = 0;
	int borderBottom = 20;
	...
	int maxPeaks = 100;
	public static Image bufferToClip=null;
	public static int    myPairId=0;
	public static String myComment="";
	
	static Logger logger = Logger.getLogger(SpectrumCanvas.class.getName());
	...
	ArrayList<ArrayList<Double>> userDeltaHits;
	public double userDeltaMass=61.956311;
	...
	boolean showMarkerIons=false;//frankp 20070405
	boolean showMasses=true;//frankp 20070405
	boolean showuserDelta=false;//frankp 20070427
	...
	Color lightgray= new Color(getDisplay(), 111, 111, 111 );//frankp 20070405
	Color lightlightgray= new Color(getDisplay(), 200, 200, 200 );//frankp 20070405
	Color lightblue = new Color(getDisplay(), 0, 0, 200 );//frankp 20070405
	...
	Color userDeltaColor  = new Color(getDisplay(), 0, 0,  155 ); 
	...
	Color fontColorred = new Color(getDisplay(),0,0,255);
	...
	public SpectrumCanvas(Composite parent, int style) {
		super(parent, style);
	}
	
	public void setDiscoveryEdition(boolean isDiscoveryEdition){//frankp new 20070426
		this.isDiscoveryEdition=isDiscoveryEdition;
	}
	...
	public void setShowMarkerIons(boolean inValue){
		this.showMarkerIons=inValue;
		this.redraw();
	}

	public void setShowMasses(boolean inValue){
		this.showMasses=inValue;
		this.redraw();
	}

	public void setShowUserDelta(boolean inValue){
		this.showuserDelta=inValue;
		this.redraw();
	}
	
	public void setUserDeltaMass(double inValue){
		this.userDeltaMass=inValue;
	}
	...
	private void recalculate(){
		myPairId=currentPair.pair_id;
		myComment=currentPair.comment;
		...
		this.userDeltaHits = new ArrayList<ArrayList<Double>>();


		this.massToChargeMax=Math.max(this.currentPair.spectrumA.precursorMass, this.currentPair.spectrumB.precursorMass);
		this.massToChargeMax=Math.max(this.massToChargeMax, this.currentPair.spectrumA.maxMZ);//bizarre but might happen
		this.massToChargeMax=Math.max(this.massToChargeMax, this.currentPair.spectrumB.maxMZ);//....

		if(this.currentPair.spectrumA.charge>2){//prevent too much emptiness in the display - set the max boundary a bit lower
			//was an issue for charge 3, 4 or more
			if(this.currentPair.spectrumA.maxMZ<this.massToChargeMax/3 && this.currentPair.spectrumB.maxMZ<this.massToChargeMax/3){
				this.massToChargeMax=this.massToChargeMax/3;
			}
			else if(this.currentPair.spectrumA.maxMZ<this.massToChargeMax/2 && this.currentPair.spectrumB.maxMZ<this.massToChargeMax/2){
				this.massToChargeMax=this.massToChargeMax/2;
			}
			else if(this.currentPair.spectrumA.maxMZ<this.massToChargeMax*2/3 && this.currentPair.spectrumB.maxMZ<this.massToChargeMax*2/3){
				this.massToChargeMax=this.massToChargeMax*2/3;
			}
		}
		this.massToChargeMin=0;
		...
		if(this.deltaMass>0){
			logger.fatal("SpectrumCanvas: deltamass not negative:"+this.deltaMass);
			return;
		}

		isPhosphoPair=false;//build 61
		//20070127 US patent 7138228 double tmpcrap=Math.min(   Math.abs(this.deltaMass+79.966331) , Math.abs(this.deltaMass+79.956815) )   ;
		...
		for(MSMS msms : msmsA){
			...
			ArrayList<Double> thisuserDeltaHits = new ArrayList<Double>();//build 61
			...
			for(MSMS msmsOther: msmsB){
				...
				if( Math.abs(msms.massToCharge - msmsOther.massToCharge + this.userDeltaMass ) < this.record.getMsmsPrecision()){
					thisuserDeltaHits.add(msmsOther.massToCharge);
					seen=true;
				}
			}
...
//			build 65 new below
			...
			userDeltaHits.add(thisuserDeltaHits);
		}
	...
	private void paintBuffer(){
		...
		//frankp 20070405  TODO: random order, switch on 
		if(this.showMasses){//draw the m/z labels
			Random randGen = new Random();//frankp 20070426
			gc.setFont(smallFont);
			for(MSMS msms: msmsA){
				...
				/*if(!isDiscoveryEdition){
					//in 25% of cases: hide the masses for personal edition users (to irritate them)
					if (randGen.nextDouble()<0.25){
						gc.setBackground(lightlightgray);//this makes the stuff unreadable
						gc.setForeground(lightlightgray);
					}
				}*/

				gc.drawString(String.format("%.2f",msms.massToCharge), xPosA, groundline-lineHeightA-gc.getFontMetrics().getHeight());
				gc.setForeground( lowerSpectrumColor );
			}
			gc.setFont(largeFont);
		}

		for(MSMS msms: msmsA){//draw the spectrum lines
			int lineHeightA = (int)((double)spectrumHeight * 2 * (msms.intensity / (2 * intensityMaxA))  ) ;
			int xPosA = (int)(bWidth * ( (msms.massToCharge - this.massToChargeMin ) / deltaMZ));	
			int groundline = spectrumHeight + this.topBarHeight;
		...
		if(currentPair.spectrumA.proteinAsc==null){//do not print protein and peptide info
			gc.drawString("" + this.currentPair.spectrumA.title + "         " + String.format("%.5f",this.currentPair.spectrumA.precursorMass) + " [Da]" + "     z=" + this.currentPair.spectrumA.charge, 2 ,this.topBarHeight + 2);
		}
		else{//print protein and peptide info 
			gc.drawString("" + this.currentPair.spectrumA.title + "         " + String.format("%.5f",this.currentPair.spectrumA.precursorMass) + " [Da]" + "     z=" + this.currentPair.spectrumA.charge + "    protein:" + currentPair.spectrumA.proteinAsc + "    peptide:" + currentPair.spectrumA.pepSequence + " "+currentPair.spectrumA.pepStart+"-"+currentPair.spectrumA.pepEnd , 2 ,this.topBarHeight + 2);	
			if(currentPair.spectrumA.pepMod!=null && currentPair.spectrumA.pepMod.length()>1){
				gc.drawString(""+this.currentPair.spectrumA.pepMod, 2, this.topBarHeight+gc.getFontMetrics().getHeight()+4);
			}
		}
		
		if(currentPair.spectrumB.proteinAsc==null){//do not print protein and peptide info
			gc.drawString("" + this.currentPair.spectrumB.title + "         " + String.format("%.5f",this.currentPair.spectrumB.precursorMass) + " [Da]" + "     z=" + this.currentPair.spectrumB.charge, 2 ,this.topBarHeight + this.seperatorBarHeight + spectrumHeight + 3 +spectrumHeight -  gc.getFontMetrics().getHeight()-3);
		}
		else{//print protein and peptide info
			gc.drawString("" + this.currentPair.spectrumB.title + "         " + String.format("%.5f",this.currentPair.spectrumB.precursorMass) + " [Da]" + "     z=" + this.currentPair.spectrumB.charge + "    protein:" + currentPair.spectrumB.proteinAsc + "    peptide:" + currentPair.spectrumB.pepSequence + " "+currentPair.spectrumB.pepStart+"-"+currentPair.spectrumB.pepEnd, 2 ,this.topBarHeight + this.seperatorBarHeight + spectrumHeight + 3 +spectrumHeight -  gc.getFontMetrics().getHeight()-3);
			if(currentPair.spectrumB.pepMod!=null && currentPair.spectrumB.pepMod.length()>1){
				gc.drawString("modification: " +this.currentPair.spectrumB.pepMod, 2,this.topBarHeight + this.seperatorBarHeight + spectrumHeight + 3 +spectrumHeight -  2*gc.getFontMetrics().getHeight()-3 -2);
			}
			else{
				//pepMod empty
			}
		...
		if(currentPair.hasWeakDeltaSignal){weakDeltaString="    [weak delta signal]";}
		
		if(currentPair.pair_id>0){
			if(myComment==null){myComment="";}
			gc.drawString( "\u0394m = " + String.format("%.5f",Math.abs(this.deltaMass)) + " [Da]   pair_id:"+currentPair.pair_id+weakDeltaString+"  "+myComment, 2 ,this.topBarHeight + spectrumHeight + (int)(((int)(this.seperatorBarHeight / 2))-gc.getFontMetrics().getHeight()/2));
		}
		else{
			gc.drawString( "\u0394m = " + String.format("%.5f",Math.abs(this.deltaMass)) + " [Da]"+weakDeltaString, 2 ,this.topBarHeight + spectrumHeight + (int)(((int)(this.seperatorBarHeight / 2))-gc.getFontMetrics().getHeight()/2));
		}
		...
		if(showMasses){
			...
			gc.setFont(smallFont);
			for(MSMS msms: msmsB){//draw masses
				...
				gc.drawString(String.format("%.2f",msms.massToCharge), xPosB, groundline+lineHeightB);
				gc.setForeground( lowerSpectrumColor );
		...
		if(showMarkerIons){
//			frankp 20070405 marker ion drawing
			Random randGen = new Random();//20070426
			gc.setFont(smallFont);
			boolean isMarker=true;//set to false if we find an equal or deltachannel
			double deltaMassHalf=deltaMass/2.0;
			for(MSMS msms2: msmsB){//draw masses
				isMarker=true;
				for(MSMS msms1: msmsA){	
					if(Math.abs(msms1.massToCharge-msms2.massToCharge) < record.getMsmsPrecision()){
						isMarker=false;
						break;//get ouf of innermost loop to save time
					}//Good to know: deltaMass is negative because spectrumA always lighter than spectrumB
					else if(Math.abs(-msms1.massToCharge+msms2.massToCharge+deltaMass) < record.getMsmsPrecision()){
						isMarker=false;
						break;//get ouf of innermost loop to save time
					}
					else if(Math.abs(-msms1.massToCharge+msms2.massToCharge+deltaMassHalf) < record.getMsmsPrecision()){
						isMarker=false;
						break;//get ouf of innermost loop to save time
					}
				}
				if(isMarker){
					int lineHeightB = (int)((double)spectrumHeight *  2 * ( msms2.intensity / ( 2 * intensityMaxB )));
					int xPosB = (int)( bWidth * (( msms2.massToCharge - this.massToChargeMin ) / deltaMZ ));
					int groundline = spectrumHeight + this.topBarHeight + this.seperatorBarHeight;

					gc.setForeground(fontColorred);
					gc.setBackground(diagramBackgroundColor);

					if(!isDiscoveryEdition){
						//in 50% of cases: hide the masses for personal edition users (to irritate them)
						if (randGen.nextDouble()<0.50){
							gc.setForeground(lightlightgray);//this makes the stuff unreadable
							gc.setBackground(lightlightgray);//this makes the stuff unreadable
						}
					}


					gc.drawString(String.format("%.2f",msms2.massToCharge), xPosB, groundline+lineHeightB);
					gc.drawLine(xPosB-4, groundline+lineHeightB-4, xPosB+4, groundline+lineHeightB+4);
					gc.drawLine(xPosB-4, groundline+lineHeightB+4, xPosB+4, groundline+lineHeightB-4);
					gc.setForeground( lowerSpectrumColor );	
				}
			}
		...
		//if(showMarkerIons || showMasses){
		if(showMarkerIons){
			if(!isDiscoveryEdition){
				gc.setBackground(diagramBackgroundColor);
				gc.setForeground(lightgray);
				gc.drawString("IN DISCOVERY EDITION, YOU CAN SEE ALL M/Z LABELS", 2, 3*bHeight/4);
			}
		}

		gc.setForeground( lowerSpectrumColor );	
		for(MSMS msms: msmsB){//draw the spectrum on top of the masses
			int lineHeightB = (int)((double)spectrumHeight *  2 * ( msms.intensity / ( 2 * intensityMaxB )));
			int xPosB = (int)( bWidth * (( msms.massToCharge - this.massToChargeMin ) / deltaMZ ));
			int groundline = spectrumHeight + this.topBarHeight + this.seperatorBarHeight;		
		...
		if(this.showuserDelta){
			for(int i = 0; i < this.userDeltaHits.size(); i++ ){
				ArrayList<Double> userDeltaHits = this.userDeltaHits.get(i);
				int startXPos = (int)(bWidth * ( (msmsA.get(i).massToCharge - this.massToChargeMin ) / deltaMZ));
				int groundline = spectrumHeight + this.topBarHeight;
				int endline = spectrumHeight + this.topBarHeight + this.seperatorBarHeight;
				for(Double hit: userDeltaHits){
					int endXPos = (int)(bWidth * ( (hit - this.massToChargeMin ) / deltaMZ));
					gc.setForeground(this.userDeltaColor);
					gc.drawLine(startXPos, groundline , endXPos, endline );
				}
			}	
		}
		if(! this.isDiscoveryEdition)
		{//20080101 draw green cross all about
			gc.setForeground(this.equalHitColor);
			gc.drawLine(0,0, bWidth, bHeight);
			gc.drawLine(0, bHeight,bWidth,0);gc.setForeground(this.equalHitColor);
			gc.drawString("The green cross is not present in Discovery Edition", 5 , bHeight/4);
		}
	...
	public static void imageToClipboard() {
		//convert incomming swt image to an awt image first
		
		logger.info("SpectrumCanvas:imageToClipboard");
		try{
		if(bufferToClip != null){
			org.eclipse.swt.graphics.ImageData imData = bufferToClip.getImageData();
			...
			logger.error("SpectrumCanvas:imageToClipboard:"+e.toString());
		}
	}
	...
	public static int getWidth(){
		if(bufferToClip!=null){
			return bufferToClip.getImageData().width;
		}
		else{
			return 10;
		}
	...
	public static int getHeight(){
		if(bufferToClip!=null){
			return bufferToClip.getImageData().height;
		}
		else{
			return 10;
		}
	...
	public static java.awt.Image getAwtImage() {
		java.awt.Image awtImage;
		awtImage=null;
		logger.info("getAwtImage");
		if(bufferToClip != null){
			org.eclipse.swt.graphics.ImageData imData = bufferToClip.getImageData();
			java.awt.image.BufferedImage buffIm = Snippet156.convertToAWT(imData);
			awtImage=(java.awt.Image)buffIm;
			...
			logger.warn("SpectrumCanvas:getAwtImage:bufferToClip is null");
		}
		return awtImage;
	}
