#!/usr/local/bin/perl
###############################################################################
#Copyright by Detectorvision AG Zurich, Switzerland
#mascot_2_deltaMasses_exporter.pl
#version: 0.9
#perl C:\detectorvision\scripts\mascot_2_deltaMasses_exporter.pl  "<resultfilepath>" "<datafilepath>" "<datafilename>"
###############################################################################
use LWP::Simple;
...
my $LOGFILE='C:/detectorvision/scripts/mascot_exporter.log';
#Directory to where the .mgf and .xml files are exported
my $MASCOT_EXPORT_DIRECTORY='C:/detectorvision/mascot_export';
###############################################################################
my $MASCOT_SERVER_CGI = $MASCOT_SERVER . '/cgi/';


#SYSTEM CHECK##################################################################
open(LOG,">>$LOGFILE") or die "cannot create or open logfile $LOGFILE\n";
print LOG "------------------------------------------------------------\n";
...
if(! -e $MASCOT_EXPORT_DIRECTORY){
	print "Mascot Export Directory $MASCOT_EXPORT_DIRECTORY does not exist\n";
	exit(0);	
}
my $numArgs=$#ARGV+1;
print      "number of arguments:$numArgs\n";
if($numArgs==0){
	print "\n----------------------\ntesting Mascot Server\n";
	my $contentTest = get $MASCOT_SERVER;
    die "Couldn't get $MASCOT_SERVER" unless defined $contentTest;
    print LOG "fetching MASCOT_SERVER $MASCOT_SERVER OK\n";
	print "validation OK";
	exit(0);
}
###############################################################################
